"""
Google search  .gs <query>
Google Image Search  .gi <query>
Google Reverse search  .grs
For all user
...
@borg.on(events.NewMessage(pattern=r"\.gs ?(.*)",incoming=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
...
@borg.on(events.NewMessage(pattern=r"\.gi ?(.*)",incoming=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
...
@borg.on(events.NewMessage(pattern=r"\.grs ?(.*)",incoming=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await event.reply("Found Google Result. Pouring some soup on it!")
        headers = {

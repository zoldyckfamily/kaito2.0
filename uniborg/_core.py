...
async def send_plug_in(event):
    ...
    await event.edit("plugin is upload {} in {} seconds".format(input_str, time_taken_in_ms))
    await asyncio.sleep(DELETE_TIMEOUT)
    await event.delete()
    ...
    if event.reply_to_msg_id:
        try:
            ...
            if "(" not in downloaded_file_name:
                ...
                await event.edit(" Instulled Plugin `{}`".format(os.path.basename(downloaded_file_name)))
            else:
                os.remove(downloaded_file_name)
                await event.edit("Plugin is not instaling 😕.")
        except Exception as e:  # pylint:disable=C0103,W0703
